...
- "accommodation_booking"
  ...
  "예약할만한 곳 추천해줘", "숙소 골라줘" 등 검색/추천 요청에 적합

- "booking_action"
  사용자가 숙소나 항공권을 실제로 예약해달라고 요청할 때 사용하는 도구
  이 도구는 검색과 예약을 모두 자체적으로 수행하므로, 이전 검색 여부와 상관없이 사용 가능
  구분 기준: 사용자가 단순히 정보를 원하는 것(accommodation_booking)인지,
  실제로 예약 행위 자체를 원하는 것(booking_action)인지를 맥락으로 판단하면 된다.
  예: "도쿄 호텔 예약해줘" → booking_action
  예: "예약할만한 숙소 찾아줘" → accommodation_booking (정보 요청)
  사용자의 예약 요청이 항공권인지 숙소인지 정확하게 구분해야 한다.
  예: "이 항공권 예약할게" → booking_action (항공권 예약)
  예: "이 호텔 예약할게" → booking_action (숙소 예약)
...
5)
이미 도시/날짜/대략의 예산 등이 정해진 상태에서 "숙소를 골라줘", "예약할만한 곳 추천" 등 숙소 정보를 찾아달라는 요청이면

→ intent = "accommodation_booking"


5-1)
사용자가 실제로 예약을 진행해달라는 느낌이면 (정보 검색이 아니라 예약 행위 자체를 요청)
예: "호텔 예약해줘", "항공권 예약 진행해줘", "이거 예약할게"
