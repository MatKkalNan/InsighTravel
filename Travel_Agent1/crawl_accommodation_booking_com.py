...
import requests
import sys
from dotenv import load_dotenv # 패키지 임포트
...
def get_destination_id(location_name: str):
    ...
    try:
        ...
        if isinstance(data, list) and len(data) > 0:
            ...
            print(f"✅ 성공! (ID: {dest_id}, Type: {dest_type})")
            ...
            if isinstance(data2, list) and len(data2) > 0:
                ...
                dest_type = data2[0].get("dest_type")
                print(f"✅ 성공! (ID: {dest_id}, Type: {dest_type})")
                return dest_id, dest_type
            else:
                print("❌ 완전히 실패.")
...
def search_hotels_api(location, check_in, check_out, guests=2):
    ...
    try:
        ...
        # Booking.com API 구조에 따라 결과 키가 다를 수 있음 (보통 result)
        ...
        for hotel in hotels[:10]: 
            try:
                ...
                item = {
                    "source": "Booking.com", # 출처 명시
                    "name": hotel.get('hotel_name', '이름 없음'),
                    "price": price_text,
                    "rating": str(hotel.get('review_score', 'N/A')),
